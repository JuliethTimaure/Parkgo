...
const register = async (req, res) => {
    let { rut, nombre, apellido, correo, contrasena, id_comuna, calle, numero } = req.body;
    ...
    try {
        ...
        const result = await db.query(
            `INSERT INTO usuario (rut, nombre, apellido, correo, contrasena, id_comuna, calle, numero) 
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id_usuario, nombre`,
            [rut, nombre, apellido, correo, hashPassword, id_comuna, calle, numero]
        );
        res.status(201).json({ message: 'Usuario registrado', user: result.rows[0] });
    } catch (err) {
        console.error(`❌ [ERROR]`, err.message);
        if (err.code === '23505') return res.status(409).json({ error: 'El RUT ya existe.' });
...
const login = async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ id: user.id_usuario, nombre: user.nombre }, process.env.JWT_SECRET || 'secret', { expiresIn: '24h' });
    ...
    try {
        const result = await db.query(
            `SELECT id_usuario, rut, nombre, apellido, correo, telefono, 
                    id_comuna, calle, numero, url_foto_perfil 
...
const updateProfile = async (req, res) => {
    const { nombre, apellido, telefono, id_comuna, calle, numero, password } = req.body;
    const client = await db.pool.connect();
    try {
        await client.query('BEGIN');
        await client.query(
            `UPDATE usuario SET nombre = $1, apellido = $2, telefono = $3, id_comuna = $4, calle = $5, numero = $6 WHERE id_usuario = $7`,
            [nombre, apellido, telefono, id_comuna, calle, numero, req.user.id]
        );
...
// 5. UPDATE AVATAR (Cloudinary Check)
const updateAvatar = async (req, res) => {
    ...
    try {
        ...
        // Verifica que se suba y recorte
